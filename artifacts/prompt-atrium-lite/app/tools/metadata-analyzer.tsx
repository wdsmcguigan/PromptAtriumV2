...
function Row({ label, value }: { label: string; value?: string | number }) {
  ...
  return (
    <View style={[styles.row, { borderTopColor: colors.border }]}>
      ...
      <Text style={[styles.rowValue, { color: colors.foreground }]} selectable>{String(value)}</Text>
    </View>
  );
}
...
export default function MetadataAnalyzerScreen() {
  ...
  async function onPick() {
    ...
    try {
      ...
      const extracted = extractMetadataFromBase64({ base64, fileName: asset.name, fileSize: asset.size ?? undefined, mimeType: asset.mimeType });
      setMeta(extracted);
    } catch (e) {
  ...
  return (
    <ScrollView style={{ backgroundColor: colors.background }} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <Text style={[styles.lead, { color: colors.mutedForeground }]}>
        Read the AI generation data embedded in an image — prompt, model, and settings.
      </Text>

      <Pressable onPress={onPick} style={[styles.pick, { backgroundColor: colors.input, borderColor: colors.border, borderRadius: colors.radius + 2 }]}>
        {uri ? (
          <Image source={{ uri }} style={styles.preview} contentFit="cover" />
        ) : (
          <View style={styles.pickEmpty}>
            <Feather name="image" size={24} color={colors.mutedForeground} />
            <Text style={[styles.pickText, { color: colors.mutedForeground }]}>Tap to choose an image file</Text>
          </View>
        )}
      </Pressable>

      {uri ? (
        <Pressable onPress={onPick} style={({ pressed }) => [styles.button, { backgroundColor: colors.secondary, opacity: pressed ? 0.85 : 1, borderRadius: colors.radius + 2 }]}>
          <Feather name="refresh-ccw" size={15} color={colors.secondaryForeground} />
          <Text style={[styles.buttonText, { color: colors.secondaryForeground }]}>Choose another</Text>
        </Pressable>
      ) : null}

      {busy ? <Text style={[styles.lead, { color: colors.mutedForeground, textAlign: "center" }]}>Reading metadata…</Text> : null}
      ...
      {meta ? (
        <>
          <View style={[styles.statusCard, { backgroundColor: colors.card, borderColor: meta.isAIGenerated ? colors.primary : colors.border, borderRadius: colors.radius + 4 }]}>
            <Feather name={meta.isAIGenerated ? "cpu" : "help-circle"} size={18} color={meta.isAIGenerated ? colors.primary : colors.mutedForeground} />
            <Text style={[styles.statusText, { color: colors.foreground }]}>
              {meta.isAIGenerated ? `AI generated — ${GENERATOR_LABELS[meta.aiGenerator]}` : "No AI generation data detected"}
            </Text>
          </View>
          ...
          {meta.prompt ? (
            <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border, borderRadius: colors.radius + 4 }]}>
              ...
              <Text style={[styles.promptText, { color: colors.foreground }]} selectable>{meta.prompt}</Text>
              {meta.negativePrompt ? <Text style={[styles.negative, { color: colors.mutedForeground }]} selectable>Negative: {meta.negativePrompt}</Text> : null}
              <View style={styles.actions}>
                <Pressable onPress={onCopyPrompt} style={({ pressed }) => [styles.smallBtn, { backgroundColor: colors.secondary, opacity: pressed ? 0.8 : 1 }]}>
                  <Feather name={copied ? "check" : "copy"} size={15} color={colors.secondaryForeground} />
                  <Text style={[styles.smallBtnText, { color: colors.secondaryForeground }]}>{copied ? "Copied" : "Copy"}</Text>
                </Pressable>
                <Pressable onPress={onSavePrompt} disabled={saved} style={({ pressed }) => [styles.smallBtn, { backgroundColor: saved ? colors.muted : colors.primary, opacity: pressed ? 0.85 : 1 }]}>
                  <Feather name={saved ? "check" : "bookmark"} size={15} color={saved ? colors.mutedForeground : colors.primaryForeground} />
                  <Text style={[styles.smallBtnText, { color: saved ? colors.mutedForeground : colors.primaryForeground }]}>{saved ? "Saved" : "Save to library"}</Text>
                </Pressable>
              </View>
            </View>
          ) : null}
...
const styles = StyleSheet.create({
  ...
  button: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingVertical: 11 },
