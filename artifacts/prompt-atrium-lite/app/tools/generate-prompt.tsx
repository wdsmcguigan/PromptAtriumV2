...
export default function GeneratePromptScreen() {
  ...
  async function onGenerate() {
    ...
    try {
      const character = selectedPreset
        ? { name: selectedPreset.name, description: selectedPreset.description ?? undefined }
        : undefined;
  ...
  async function onSave() {
    ...
    await liteAdapter.create({
      ...
      description: "Generated with PromptAtrium Lite",
      promptType: selectedTemplate?.template_type ?? undefined,
      isPublic: false,
    });
    setSaved(true);
  }
  ...
  return (
    ...
    >
      ...
      <TextInput
        ...
        multiline
        style={[styles.input, { backgroundColor: colors.input, color: colors.foreground, borderColor: colors.border, borderRadius: colors.radius + 2 }]}
      />
        ...
        <View style={styles.group}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chips}>
            ...
            {templates.data.map((t: PromptTemplate) => (
              <Chip key={t.id} label={t.name} active={templateId === t.id} onPress={() => setTemplateId(templateId === t.id ? null : t.id)} />
            ))}
          </ScrollView>
        </View>
      ) : null}
        ...
        <View style={styles.group}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chips}>
            ...
            {presets.data.map((p: CharacterPreset) => (
              <Chip key={String(p.id)} label={p.name} active={presetId === String(p.id)} onPress={() => setPresetId(presetId === String(p.id) ? null : String(p.id))} />
            ))}
          </ScrollView>
        </View>
      ) : null}
      ...
      <View style={[styles.switchRow, { borderColor: colors.border, borderRadius: colors.radius + 2 }]}>
        <View style={styles.switchText}>
          ...
          <Text style={[styles.switchSub, { color: colors.mutedForeground }]}>Add enthusiastic, descriptive flourishes</Text>
        </View>
        <Switch value={happyTalk} onValueChange={setHappyTalk} trackColor={{ true: colors.primary, false: colors.secondary }} />
      </View>

      <Pressable
        onPress={onGenerate}
        disabled={!idea.trim() || loading}
        style={({ pressed }) => [styles.button, { backgroundColor: colors.primary, opacity: !idea.trim() || loading ? 0.5 : pressed ? 0.85 : 1, borderRadius: colors.radius + 2 }]}
      >
        {loading ? <ActivityIndicator color={colors.primaryForeground} /> : (
          <>
            <Feather name="zap" size={16} color={colors.primaryForeground} />
            <Text style={[styles.buttonText, { color: colors.primaryForeground }]}>Generate Prompt</Text>
          </>
        )}
      </Pressable>
      ...
      {result ? (
        <View style={[styles.resultCard, { backgroundColor: colors.card, borderColor: colors.border, borderRadius: colors.radius + 4 }]}>
          <Text style={[styles.resultLabel, { color: colors.mutedForeground }]}>Enhanced prompt</Text>
          <Text style={[styles.resultText, { color: colors.foreground }]} selectable>{result}</Text>
          <View style={styles.resultActions}>
            <Pressable onPress={onCopy} style={({ pressed }) => [styles.smallBtn, { backgroundColor: colors.secondary, opacity: pressed ? 0.8 : 1 }]}>
            ...
            <Pressable onPress={onSave} disabled={saved} style={({ pressed }) => [styles.smallBtn, { backgroundColor: saved ? colors.muted : colors.primary, opacity: pressed ? 0.85 : 1 }]}>
              <Feather name={saved ? "check" : "bookmark"} size={15} color={saved ? colors.mutedForeground : colors.primaryForeground} />
              <Text style={[styles.smallBtnText, { color: saved ? colors.mutedForeground : colors.primaryForeground }]}>{saved ? "Saved to library" : "Save to library"}</Text>
            </Pressable>
          </View>
        </View>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  input: { borderWidth: 1, padding: 14, fontSize: 15, fontFamily: "Inter_400Regular", minHeight: 96, textAlignVertical: "top" },
  group: { gap: 8 },
  groupLabel: { fontSize: 13, fontFamily: "Inter_600SemiBold" },
  chips: { paddingRight: 8 },
  switchRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", borderWidth: 1, padding: 14, gap: 12 },
